...
class ReadingGoalViewTest {
    ...
    void countOnlyReadBooksWithAFinishDateTowardsGoal() {
        ...
        assertSoftly(softly -> {
            softly.assertThat(howManyReadThisYear(BOOKS, readShelf)).isEqualTo(READ_BOOKS_TO_ADD);
            softly.assertThat(howManyReadThisYear(PAGES, readShelf)).isEqualTo(pagesReadInReadShelf);
        });
    }
